                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @foreach($countries as $key1=>$val1)
                                                @if($val1['id']==$row->country_id)
                                                    {{$val1['name']}}
                                                @endif
                                            @endforeach
                                        </td>

                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $row->purpose}}</td>
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div><label class='block'><span class='text-gray-700 @error(' country_id')
                                            text-red-500 @enderror'>Country</span> <select wire:model="country_id"
                                                                                           class='mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring
                                                focus:ring-indigo-200 focus:ring-opacity-50'>
                                                <option>Select</option>
                                                @foreach($countries as $key2=>$val2)
                                                    <option value="{{$val2['id']}}">{{$val2['name']}}</option>
                                                @endforeach
                                            </select>@error('country_id')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
                                    <div><label class='block'><span class='text-gray-700 @error(' purpose')
                                            text-red-500 @enderror'>Purpose</span><input type='text'
                                                                                         class='mt-1 block w-full rounded-md border-gray-300 shadow-sm @error('
                                                                                         purpose') border-red-500
                                            @enderror focus:border-indigo-300 focus:ring focus:ring-indigo-200
                                            focus:ring-opacity-50' wire:model='purpose'>@error('purpose')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
