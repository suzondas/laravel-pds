                            ...
                            <table class="w-full divide-y divide-gray-200">
                                ...
                                @forelse($rows as $key=>$row)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $langauge_helper[$row->language]}}</td>

                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $langauge_skill_helper[$row->read]}}</td>

                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{$langauge_skill_helper[$row->write]}}</td>

                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $langauge_skill_helper[$row->speak]}}</td>
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div><label class='block'><span class='text-gray-700 @error(' language')
                                            text-red-500 @enderror'>Language</span><select
                                                    class='mt-1 block w-full rounded-md border-gray-300 shadow-sm
                                            focus:border-indigo-300 focus:ring focus:ring-indigo-200
                                            focus:ring-opacity-50' wire:model='language'>
                                                <option>Select</option>
                                                @foreach($langauge_helper as $key=>$val)
                                                    <option value="{{$key}}">{{$val}}</option>
                                                @endforeach
                                            </select>@error('language')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
                                    <div><label class='block'><span class='text-gray-700 @error(' read') text-red-500
                                            @enderror'>Read</span><select
                                                    class='mt-1 block w-full rounded-md border-gray-300 shadow-sm
                                            focus:border-indigo-300 focus:ring focus:ring-indigo-200
                                            focus:ring-opacity-50' wire:model='read'>
                                                <option>Select</option>
                                                @foreach($langauge_skill_helper as $key=>$val)
                                                    <option value="{{$key}}">{{$val}}</option>
                                                @endforeach
                                            </select>@error('read')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
                                    <div><label class='block'><span class='text-gray-700 @error(' write') text-red-500
                                            @enderror'>Write</span><select
                                                    class='mt-1 block w-full rounded-md border-gray-300 shadow-sm
                                            focus:border-indigo-300 focus:ring focus:ring-indigo-200
                                            focus:ring-opacity-50' wire:model='write'>
                                                <option>Select</option>
                                                @foreach($langauge_skill_helper as $key=>$val)
                                                    <option value="{{$key}}">{{$val}}</option>
                                                @endforeach
                                            </select>@error('write')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
                                    <div><label class='block'><span class='text-gray-700 @error(' speak') text-red-500
                                            @enderror'>Speak</span><select
                                                    class='mt-1 block w-full rounded-md border-gray-300 shadow-sm
                                            focus:border-indigo-300 focus:ring focus:ring-indigo-200
                                            focus:ring-opacity-50' wire:model='speak'>
                                                <option>Select</option>
                                                @foreach($langauge_skill_helper as $key=>$val)
                                                    <option value="{{$key}}">{{$val}}</option>
                                                @endforeach
                                            </select>@error('speak')<span
                                                    class='text-red-500 text-sm'>{{ $message }}</span>@enderror</label>
                                    </div>
